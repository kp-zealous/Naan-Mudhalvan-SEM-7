        ...
        try {
            ...
            if(usertype === 'restaurant'){
                const newUser = new User({
                    username, email, usertype, password, approval: 'pending'
                });
                ...
                const newUser = new User({
                    username, email, usertype, password, approval: 'approved'
                });
...
// Remove any unwanted spaces before comparison (just in case)
...
// Fetch restaurant details with all food items in the menu
app.get('/fetch-restaurant-details-with-items/:restaurantId', async (req, res) => {
    const { restaurantId } = req.params;
    
    try {
        // Find the restaurant by ID and populate the 'menu' field with food items
        ...
        // Return restaurant details along with its food items (from the populated 'menu')
        res.json(restaurant);
    } catch (err) {
        console.error(err);
        res.status(500).json({ message: 'Error occurred while fetching restaurant details with items' });
    }
});
